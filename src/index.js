...
const interpreter = (options = {}) => {
  ...
  const evaluate = (form, context = {}) => {
    trace('evaluate: evaluating:', JSON.stringify(form, null, 2))
    if (Array.isArray(form)) {
      // eslint-disable-next-line no-use-before-define
      return evaluateArrayForm(form, context)
    } else if (typeof form === 'object') {
      // eslint-disable-next-line no-use-before-define
      return evaluateObjectForm(form, context)
    } else if (isSymbol(form)) {
      trace(`evaluate: replacing "${form}"/"${symbolToString(form)}" value from context or registry`)
  ...
  const evaluateArrayForm = (sform, context) => {
    ...
    if (typeof evaluated[0] === 'function') {
      ...
      return evaluated[0].call(undefined, evaluated, context, trace, evaluate)
    }
  ...
  const evaluateObjectForm = (oform, context) => {
    // special forms are called with their args unevaluated:
    ...
    if (symbol) {
      if (specialForms[symbol]) {
        return specialForms[symbol](oform, context, trace, evaluate)
        ...
        keys.forEach(key => {
          oform[key] = evaluate(oform[key], context)
        })

        trace(`oform: calling ${symbol}`)

        return registry[symbol].call(undefined, oform, context, trace, evaluate)
      }
